...
func computeInputHash(ctx context.Context, taskData TaskData) (string, error) {
	if taskData == nil {
...
func errorPayloadFromError(err error, inputRef *InputReference) (json.RawMessage, string, error) {
	...
	appErr = AppError{Payload: AppErrorPayload{Message: err.Error(), Level: "error", InputRef: inputRef}}
	raw, tdErr := json.Marshal(appErr.Payload)
	return json.RawMessage(raw), payloadKindAppError, tdErr
}

func artifactHash(ctx context.Context, art Artifact) (string, error) {
	return art.Sha256(ctx)
}

// envelopeToTaskData returns the cached task output or the rehydrated error encoded in the envelope.
func envelopeToTaskData(env chapterEnvelope, artifacts []Artifact) (TaskData, error) {
	copiedArtifacts := make([]Artifact, 0, len(artifacts))
