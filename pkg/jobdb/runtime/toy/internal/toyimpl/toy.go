...
type JobIDGenerator func(tenantId string) (jobdb.JobKey, error)
...
func normalizeMetadataPredicates(predicates []jobdb.MetadataPredicate) ([]normalizedMetadataPredicate, error) {
	if len(predicates) == 0 {
		return nil, nil
	}
...
func NewToyEngine(opts ...Option) *ToyEngine {
	engine := &ToyEngine{
		...
		idGenerator: func(tenantId string) (jobdb.JobKey, error) {
			return jobdb.JobKey{TenantId: tenantId, JobId: ksuid.New().String()}, nil
		},
		logger: slog.Default(),
	}
...
func (e *ToyEngine) GetJobRun(ctx context.Context, req jobdb.GetJobRunRequest) (jobdb.GetJobRunResponse, error) {
	e.mu.Lock()
	record, ok := e.jobRecords[req.JobKey]
	e.mu.Unlock()
	if !ok {
		return jobdb.GetJobRunResponse{}, fmt.Errorf("job %s not found", req.JobKey)
	}
	...
	ensureAttempt := func(num int) int {
		...
		attempt := jobdb.JobAttempt{Attempt: num}
		if num == 1 && startSet {
	...
	for _, ord := range ordinals {
		...
		outcome := jobdb.TaskOutcome{
			Status:      jobdb.TaskOutcomeStatusSucceeded,
			PayloadKind: "App",
		}
		if chap.Err != nil {
			state = jobdb.TaskAttemptStateFailed
	...
	if status == jobdb.JobStatusCompleted && (result != nil || jobErr != nil) {
	...
	if status != jobdb.JobStatusCompleted && capability != "" {
		...
		state := jobdb.TaskAttemptStateWaiting
		if status == jobdb.JobStatusReady {
			state = jobdb.TaskAttemptStateReady
		} else if status == jobdb.JobStatusActive {
			state = jobdb.TaskAttemptStateRunning
		}
...
func normalizeToyJobRunOptions(req jobdb.GetJobRunRequest) (bool, bool, bool, bool) {
...
func toyTaskOutcomeFromError(err error) jobdb.TaskOutcome {
	outcome := jobdb.TaskOutcome{
		Status:      jobdb.TaskOutcomeStatusFailed,
		PayloadKind: "AppError",
		Error: &jobdb.TaskError{
			Kind:    jobdb.TaskErrorKindApp,
			Message: err.Error(),
		},
	}

	var jobFailed *jobdb.JobFailedError
...
func durationPtr(v jobdb.Duration) *jobdb.Duration {
	return &v
}

func buildToyTaskIO(ctx context.Context, data jobdb.TaskData, jobID string, ordinal int64, includeData bool, includeArtifacts bool) (*jobdb.TaskIO, error) {
...
func buildToyArtifactInfos(ctx context.Context, artifacts []jobdb.Artifact, jobID string, ordinal int64) ([]jobdb.ArtifactInfo, error) {
	if len(artifacts) == 0 {
		return nil, nil
	}
...
func (e *ToyEngine) ListJobs(ctx context.Context, req jobdb.ListJobsRequest) (jobdb.ListJobsResponse, error) {
	...
	var (
		activeStatuses []jobdb.JobStatus
		includeActive  bool
		includeArchive bool
	)
	if len(req.Statuses) > 0 {
		for _, st := range req.Statuses {
			...
			case jobdb.JobStatusCompleted:
				includeArchive = true
			case jobdb.JobStatusReady, jobdb.JobStatusExpired, jobdb.JobStatusPendingJobs, jobdb.JobStatusAwaitingFuture, jobdb.JobStatusActive, jobdb.JobStatusCrashConcern, jobdb.JobStatusCancelled:
				includeActive = true
				activeStatuses = append(activeStatuses, st)
			default:
				return jobdb.ListJobsResponse{}, fmt.Errorf("unknown status %q", st)
			}
		}
	} else {
		if len(req.Stores) == 0 {
			...
			for _, store := range req.Stores {
				...
				case jobdb.JobStoreArchived:
					includeArchive = true
				default:
					return jobdb.ListJobsResponse{}, fmt.Errorf("unknown store %q", store)
				}
			}
		}
	}
	...
	jobKeyAllowed := func(key jobdb.JobKey) bool {
	...
	statusAllowed := func(st jobdb.JobStatus) bool {
		if len(req.Statuses) == 0 {
			return true
		}
	...
	records := make([]jobdb.JobSummary, 0)
	e.mu.Lock()
	for key, rec := range e.jobRecords {
		...
		if status == jobdb.JobStatusCompleted && !includeArchive {
			rec.mu.Unlock()
			continue
		}
		if status != jobdb.JobStatusCompleted && !includeActive {
			rec.mu.Unlock()
			continue
		}
	...
	filtered := make([]jobdb.JobSummary, 0, len(records))
	...
	return jobdb.ListJobsResponse{Jobs: filtered, NextPageToken: nextToken}, nil
}

func (e *ToyEngine) getJobRecord(key jobdb.JobKey) *jobRecord {
	e.mu.Lock()
	defer e.mu.Unlock()
...
func materializeArtifacts(ctx context.Context, artifacts []jobdb.Artifact, logger *slog.Logger) ([]jobdb.Artifact, error) {
	if len(artifacts) == 0 {
		return artifacts, nil
	}

	materialized := make([]jobdb.Artifact, 0, len(artifacts))
	for _, art := range artifacts {
		// Read artifact bytes
		...
		memArt := jobdb.NewArtifactFromBytes(art.Name(), data)
...
func assignToyArtifactKeys(artifacts []jobdb.Artifact, jobID string, ordinal int64) {
	if jobID == "" || ordinal < 0 {
		return
	}
	for _, art := range artifacts {
...
func cleanupArtifacts(ctx context.Context, artifacts []jobdb.Artifact, logger *slog.Logger) {
	for _, art := range artifacts {
...
func (e *ToyEngine) OpenStoredArtifact(ctx context.Context, ref jobdb.ArtifactRef) (jobdb.ArtifactReader, error) {
	e.mu.Lock()
	...
	e.mu.Unlock()
	if ok {
		art := jobdb.NewArtifactFromBytes(ref.Name, data)
