...
func (r *Runtime) preflightScheduleLease(ctx context.Context, lease *executionLease) (bool, error) {
	...
	occ, hasSchedule, err := jobdb.ExtractScheduleOccurrenceMetadata(detail.Metadata)
	if err != nil {
		return false, err
	}
	if !hasSchedule {
	...
	row, found, err := r.loadScheduleRow(ctx, jobdb.ScheduleKey{TenantId: jobKey.TenantId, ScheduleId: occ.ScheduleId})
	if err != nil {
		return false, err
	}
	if !found {
	...
	if occ.PreviousJobId != "" {
		failureBits = jobdb.AppendScheduleFailureBit(failureBits, r.previousJobSucceeded(ctx, jobKey.TenantId, occ.PreviousJobId), windowSize)
	}
